This describes the resources that make up the official Alkeme API v1.
...
```
Accept: application/vnd.alkeme+json; version=1.0
```
...
Requests that return multiple items will be paginated to 30 items by default. You can specify page for the item with the `?page` parameter. For some resources, you can also set a custom page size up to 1000 with the `?per_page` parameter.
